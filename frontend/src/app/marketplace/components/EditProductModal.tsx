...
import type { KakaoMapState, Product, UpdateProductRequest } from "../types"
...
interface EditProductModalProps {
  ...
  onUpdate: (productId: number, data: UpdateProductRequest, images?: File[]) => Promise<void>
  isDarkMode: boolean
  kakaoMapState: KakaoMapState
}

const EditProductModal: React.FC<EditProductModalProps> = ({ isOpen, onClose, product, onUpdate, isDarkMode,kakaoMapState }) => {
  const {user} =useAuth()
  ...
  useEffect(() => {
    if (product) {
      setFormData({
        ...
        sellerId: product.sellerId, // ✅ 반드시 포함
        itemImages: product.itemImages || [], // ✅ 반드시 포함
        meetLocation: product.meetLocation,
        status: product.status,
      })
      setImagePreviews(product.itemImages || [])
    }
  }, [product])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsLoading(true)
      await onUpdate(product.itemid, formData, images)
      onClose()
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div
        className={`w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl shadow-xl ${isDarkMode ? "bg-gray-800" : "bg-white"}`}
      >
        {/* 헤더 */}
        <div
          className={`flex items-center justify-between p-6 border-b ${isDarkMode ? "border-gray-700" : "border-gray-200"}`}
        >
          <h2 className={`text-xl font-semibold ${isDarkMode ? "text-white" : "text-gray-900"}`}>상품 수정</h2>
          <button
            onClick={onClose}
              ...
              {imagePreviews.length > 0 && (
                <div className="grid grid-cols-3 gap-4">
                  {imagePreviews.map((preview, index) => (
                    <div key={`edit-preview-${index}`} className="relative">
                      <img
          ...
          <div>
            ...
            >
              ...
              {CATEGORIES.filter((cat) => cat !== "전체").map((category) => (
                <option key={`edit-category-${category}`} value={category}>
                  {category}
                </option>
              ))}
            </select>
